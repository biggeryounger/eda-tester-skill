# Optimus check_design_data Smoke case: required -file combined with the optional -netlist switch.
...
set report_dir ./out/check_design_data_netlist
...
# TEST_ACTION
pv_check_log {
    check_design_data -file $report_file -netlist
    if {![file isfile $report_file]} {
        error "check_design_data -netlist did not generate the report file: $report_file"
    }
    if {[file size $report_file] <= 0} {
        error "check_design_data -netlist generated an empty report file: $report_file"
    }
} -name check_design_data_file_netlist
